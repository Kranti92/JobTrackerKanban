...
import { COLUMN_STYLES } from '@/lib/types';
...
export default function JobCard({ job, onEdit, onDelete, overlay = false }: Props) {
  ...
  return (
    <div
      ...
      style={style}
      className={[
        'relative group rounded-xl border-l-4 p-3.5 select-none',
        cs.cardBorder,
        'bg-white dark:bg-white/[0.04] border border-l-4 border-slate-200/70 dark:border-white/[0.07]',
        overlay
          ? 'shadow-2xl shadow-brand-500/20 rotate-1 scale-[1.02]'
          : isDragging
          ? 'opacity-30'
    ...
    >
      ...
      <div
        {...attributes}
        {...listeners}
        className="absolute right-2.5 top-3 opacity-0 group-hover:opacity-100 transition-opacity cursor-grab active:cursor-grabbing text-slate-300 dark:text-slate-600 hover:text-slate-500 dark:hover:text-slate-400"
      >
